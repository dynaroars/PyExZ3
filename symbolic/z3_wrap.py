...
class Z3Wrapper(object):
    ...
    def _coneOfInfluence(self, asserts, query):
        cone = []
        cone_vars = set(query.myvars)
        ws = [a for a in asserts if len(set(a.myvars) & cone_vars) > 0]
        ...
        while len(ws) > 0:
            ...
            cone.append(a)
            new_ws = [a for a in remaining if len(
                set(a.myvars) & cone_vars) > 0]
        ...
        if res == sat:
            ...
            for a in self.asserts:
                ...
                if (not eval):
                    ...
                    break
            if (not mismatch):
                mismatch = not (not self.z3_expr.predToZ3(
                    self.query, self.solver, model))
            # print(mismatch)
            return (res, mismatch)
        elif res == unknown:
            self.solver.pop()

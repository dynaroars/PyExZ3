...
class SymbolicType(metaclass=abc.ABCMeta):
    ...
    def unwrap(self):
        return (self.concrete_val, self if self.isVariable else self.expr)
    ...
    def _eq_worker(self, expr1, expr2):
        ...
        if isinstance(expr1, list):
            return len(expr1) == len(expr2) and\
                ...
                all([self._eq_worker(x, y)
                    for x, y in zip(expr1[1:], expr2[1:])])
    ...
    def toString(self):
        if self.isVariable:
            return f"{self.name}#{self.concrete_val}"
        else:
